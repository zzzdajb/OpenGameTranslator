...
当前 Cocos2d-JS 安装属于托管修改：一定会 patch `Resources/script/jsb_boot.js`；如果 `Resources/data/project.json` 已是可解析的明文 JSON，`install-cocos` 会备份后写回替换版本。`games/maya` 这类 AGTK 样本的剧情对话尚未覆盖，需要继续验证 native hook。需要还原时运行：
