...
## 25. 2026-05-19 actionGroups 结构对比准备

本轮继续执行“用 Neruko 作为正样本，寻找 Maya 中结构相似动作链”的路线。

已完成：

- 新增 `tools/compare_agtk_dialogue_patterns.mjs`：
  - 使用现有 `opengametranslator-extracted-texts.json` 做动作名层面的离线对比。
  - 输出：
    - `output/agtk-dialogue-pattern-report.txt`
    - `output/agtk-dialogue-pattern-report.json`
  - 结论：Neruko 的 `テキスト１` / `消すテンプレ` / `出現テンプレ` 模板在 Maya 中缺失，单靠动作名相似度会把 UI/编辑器标签误判为对白链。
- 增强 `runtime/cocos2d-js/opengametranslator-probe.js`：
  - 新增 `structuralData.actionGroups`。
  - 每个 action group 会导出：
    - 所属对象路径和对象名；
    - action 序列；
    - action 内命令数量；
    - `commandTypeCounts`；
    - 关键命令类别，如 `messageShow`、`switchVariableChange` 等；
    - message/switch 变量 ID；
    - 简短脚本摘要。
- 修改 `src/engines/cocos2d-js/Cocos2dJsProbeInstaller.ts`：
  - `install-probe` 会清理旧的 `output/opengametranslator-structural.json`，避免误读 stale 输出。
- 新增 `tools/compare_agtk_action_groups.mjs`：
  - 读取 Neruko 和 Maya 的新版 `opengametranslator-structural.json`。
  - 使用 Neruko 的正样本 actionGroups 合成结构 profile。
  - 按命令结构相似度输出 Maya 候选链。
  - 如果结构文件缺失或还没有 `actionGroups`，会生成“未就绪”报告，而不是直接报错。
  - 当前输出：
    - `output/agtk-action-group-compare.txt`
    - `output/agtk-action-group-compare.json`
...
npm run dev -- install-probe games/Neruko_wa_Sodatsu Neruko_wa_Sodatsu.exe
npm run dev -- verify-probe games/Neruko_wa_Sodatsu
```

两者均显示 `Status: Installed`。旧运行时输出已经被清理，因此当前 `compare_agtk_action_groups` 报告为 `Ready: no` 是预期状态。

下一步需要用户在 Windows 宿主机运行两个样本：

```bat
cd /d C:\York\Works\Programming\OpenGameTranslator\games\Neruko_wa_Sodatsu
run-me.bat
```
...
```bash
node tools/compare_agtk_action_groups.mjs
```

下一轮重点看：

- Neruko 的正样本 actionGroups 是否稳定包含 `テキスト１` / `消すテンプレ` / `出現テンプレ` 链；
- Maya 中结构相似度最高的候选 actionGroups 是 UI、菜单，还是剧情相关链；
- 候选链中的 `messageVariableIds`、`switchVariableIds`、脚本摘要是否指向真实对白生成路径。

## 26. 2026-05-19 actionGroups 同步结果与下一版探针

用户已在 Windows 侧运行 Maya 和 Neruko，WSL 侧确认同步到新版结构文件：

- Neruko：`actionGroups = 290`
- Maya：`actionGroups = 612`

本轮复核结论：

- Neruko 的显式对白组稳定集中在 `objectList[0].children[5].children[10]`、`objectList[0].children[8].children[4]`，对象名包含 `テキスト`，动作名中混有真实对白和 `テキスト１` / `消すテンプレ` / `出現テンプレ` 模板。
- Neruko 的通用文本系统组集中在 `objectList[6].children[0].children[*]`，典型动作名为 `テキスト表示`、`テキスト枠表示・サイドメニュー表示`、`次のテキストへ（アニメーションに＋１）`。
- 初版结构相似度仍会被 Maya 的普通交互/玩法动画组误导，因为这些组也大量使用 `switchVariableChange`、`soundPlay` 等命令。
- 已收窄 `tools/compare_agtk_action_groups.mjs` 的评分：
  - 单独输出 Neruko 显式对白模板组；
  - 单独输出 Neruko 文本系统组；
  - 对 Maya 额外按 `Opening`、`*day`、`テキストフィールド表示`、`繰り返し`、`アニメーション変数＋１`、`終了・次へ` 等特征排序。

新的关键发现：

- Maya 的高可信候选不再是普通交互组，而是：
  - `objectList[14].children[0]`：`Opening`
  - `objectList[13].children[*]`：`1day`、`3day`、`5day`、`25day`、`エンディング` 等日程/剧情事件组
- 这些组反复出现同一套动作链：
  - `設定　分岐`
  - `音声再生`
  - `テキストフィールド表示`
...
- 当前结构摘要显示它们主要操作变量：
  - `2081`：`日付`
  - `18`：局部对象变量，常见名称为“アニメの表示を指定フレームで固定”
  - `2172`：局部对象变量，常见名称为“アニメーション最大フレーム数”
- 这说明 Maya 的剧情文字很可能不是标准 `messageShow` 文本，而是“文本框对象 + 动画帧/固定帧变量”驱动的显示系统。实际对白可能已经被预渲染成动画帧/图片，或藏在动作命令参数指向的对象/动画资源中。

已完成代码调整：

- `runtime/cocos2d-js/opengametranslator-probe.js` 新增 `focusedActionGroups`：
  - 只对疑似文本系统/剧情组导出更细的命令快照；
  - 包含每个重点 action 的 `commandSamples`；
  - 对 `switchVariableChange`、`objectCreate`、`objectAction`、`soundPlay` 等命令保留浅层原始参数；
  - 目标是下一轮直接看到 `テキストフィールド表示` 到底创建/切换了哪个对象、动作、动画帧或素材。
- `tools/compare_agtk_action_groups.mjs` 已更新为 focused 排序，并显示 `focusedActionGroups` 数量。
- 已重新安装并验证新版探针：
  - `games/maya`：`Status: Installed`
  - `games/Neruko_wa_Sodatsu`：`Status: Installed`

注意：`install-probe` 会清理旧的 `opengametranslator-extracted-texts.json` 和 `opengametranslator-structural.json`，因此当前需要重新运行游戏生成新版输出。

下一次用户只需在 Windows 宿主机分别运行：
...
- `focusedActionGroups` 是否存在；
- Maya 的 `Opening` / `*day` 组中，`テキストフィールド表示` 的命令参数指向了哪个对象或动作；
- 是否能沿 `objectCreate` / `objectAction` / 固定帧变量追到真实对白资源。

## 27. 2026-05-19 focusedActionGroups 首轮复核

用户已运行上一版 focused 探针，WSL 侧确认：

- Maya：`focusedActionGroups = 39`
- Neruko：`focusedActionGroups = 63`

Maya 当前关键事实：

- `Opening` 和 `*day` 组已经稳定进入 focused 输出。
- `テキストフィールド表示` 动作没有直接创建文本对象，也没有 `messageShow` 文本；它主要切全局 switch。
- 已能从现有 `extracted-texts` 反推部分全局 switch：
  - `2081`：`テキストフィールド`
  - `2082`：`テキストフィールドエンディング`
  - `2083`：`フェードイン`
  - `2084`：`画面切り替え`
  - `2055` / `2056` / `2057`：`シーン１` / `シーン２` / `シーン３`
- `objectList[15].children[2]`：`event　テキスト枠` 本身只有 7 个 action，且命令数为 0；它更像是被 switch/动画状态驱动的显示对象。
- 当前缺口：上一版 focused 输出没有导出 `actionExec` 的内部参数，也没有导出 focused 对象的 `animationId`、动画资源、图片资源，因此还不能直接判断对白是否预渲染在动画帧/图片里。

已继续增强探针：

- `runtime/cocos2d-js/opengametranslator-probe.js`
  - 新增 `switchDefinitions`，直接记录 switch 的 id/name/memo；
  - 新增 `imageDefinitions`，记录 image 的 id/name/filename/srcFilename；
  - `focusedActionGroups` 增加 `ownerFields`，用于看到 focused 对象的 `animationId` 等关键字段；
  - `commandSamples` 增加 `actionExec`、`objectChange`、`objectMove`、`objectShow`、`objectLock`、`wait` 等浅层参数；
  - 新增 `focusedAnimations`，按 focused 对象引用的 `animationId` 导出对应 animation 的 resource/motion/direction/frame 摘要。
- 新增 `tools/analyze_agtk_focused_text_system.mjs`
  - 默认读取 Maya 的 structural/extracted 输出；
  - 输出 `output/agtk-focused-text-system-report.txt/json`；
  - 汇总 focused 组 switch 使用、actionExec、focused 动画和图片引用；
  - 当前如果输出文件尚未生成，会返回 `Ready: no`，这是预期行为。
- 已重新安装并验证新版探针：
  - `games/maya`：`Status: Installed`
  - `games/Neruko_wa_Sodatsu`：`Status: Installed`

注意：本轮重新安装探针后，旧 runtime 输出再次被清理。下一步仍需用户重新运行两个样本。

下一轮同步后优先执行：

```bash
node tools/analyze_agtk_focused_text_system.mjs
node tools/compare_agtk_action_groups.mjs
```

然后重点看：

- `focusedActionGroups[].ownerFields.animationId`
- `focusedAnimations[].resourceInfoList`
- `focusedAnimations[].motionList[].directionList[].frameList`
- `imageDefinitions` 中被 frame/resource 引用的图片文件名

如果这些资源指向文本帧图片，下一步转向“资源/动画帧替换”方案；如果仍然没有文本资源，再继续回到 native hook 或图像层方案。

## 28. 2026-05-19 定位 Maya 正文图片资源

用户再次运行新版 focused 探针后，WSL 侧确认：

- Maya：`focusedAnimations = 8`
- `switchDefinitions = 6957`
- `imageDefinitions = 1128`

关键发现：

- Maya 的日程/剧情对象几乎全部共用 `animationId = 249`。
- `animationId = 249` 的动画名是 `セリフ`。
- 该动画包含：
  - `resources = 96`
  - `motions = 39`
  - 每张资源图通常是 `hdivCount = 2`、`vdivCount = 15`
  - 每个显示帧大小约 `900 x 160`
- 对应图片资源已经定位：
  - `img/1394.png` 到 `img/1422.png`：`e真面目 01` 到 `e真面目 27`
  - `img/1423.png` 到 `img/1451.png`：`e内気 01` 到 `e内気 27`
  - `img/1452.png` 到 `img/1480.png`：`e陽気 01` 到 `e陽気 27`
  - `img/1481.png` 到 `img/1488.png`：`e触ない*`
  - `img/1489.png`、`img/1490.png`：`eエンディング`
  - `img/1389.png` 到 `img/1393.png`：Opening 相关 `e出だし`
- 这些磁盘图片仍是 `enc` 头加密文件，`file` 识别为 `data`，不是普通 PNG。

结论：

- 这是 Maya 攻关中第一次真正意义上的实质性突破：此前只能证明“标准文本路径无效”，本轮首次定位到真实正文的载体和资源编号范围。
- 该结论尚不是最终可用翻译方案；还需要下一轮验证运行时能否导出明文 PNG，以及引擎是否接受托管替换后的 PNG。
- Maya 的正文大概率已经定位：不是字符串，也不是 C++ 文本渲染，而是预渲染在加密图片资源中的对白位图。
- 翻译路线应优先转为：
  1. 让游戏运行时用 `jsb.fileUtils.getDataFromFile()` 解密图片；
  2. 用 `jsb.fileUtils.writeDataToFile()` 把明文 PNG 导出到 `Resources/OpenGameTranslator/output/decrypted-img/`；
  3. 对明文 PNG 做 OCR / 手动校对 / 翻译；
  4. 生成同尺寸同切片布局的中文 PNG；
  5. 通过托管安装替换对应 `Resources/img/*.png`，保留备份和 restore 脚本。

已继续增强探针：

- `runtime/cocos2d-js/opengametranslator-probe.js`
  - 新增 `decryptedImageExports`；
  - 自动收集 focused 动画引用的 imageId；
  - 读取 `imageDefinitions` 中的 `filename`；
  - 尝试用 `getDataFromFile(filename)` 读取解密后的二进制；
  - 尝试用 `writeDataToFile(data, "OpenGameTranslator/output/decrypted-img/<name>.png")` 写出明文图片；
  - 记录二进制数据类型、长度、前 16 字节和写出结果。
- `tools/analyze_agtk_focused_text_system.mjs`
  - 修正 resource 的 imageId 映射；
  - 报告 focused 动画 resource 对应的 image name/filename；
  - 报告 `decryptedImageExports` 成功/失败数量。
...
注意：重新安装探针后旧 runtime 输出再次被清理。下一步需要用户重新运行 Maya；Neruko 不是必须，但跑它可继续做正样本对照。

下一轮同步后优先检查：

```bash
find games/maya/Resources/OpenGameTranslator/output/decrypted-img -maxdepth 1 -type f | head
node tools/analyze_agtk_focused_text_system.mjs
file games/maya/Resources/OpenGameTranslator/output/decrypted-img/1394.png
```

## 29. 2026-05-19 首次成功解密并导出对话图片

修复了探针中 `resources[ri].imageId` → `resources[ri].image` 的字段名 bug 后，重新运行游戏，成功通过 `getDataFromFile()` + `writeDataToFile()` 解密并导出全部对话图片。

关键数据点：

- **122 张图片全部解密成功**（`ok: 122, failed: 0`）
- 其中 **103 张**属于对话相关（`img/1389.png` ~ `img/1491.png`）
- 全部为标准 PNG，**无 PNG 文本元数据块**（`formatHint: "PNG"`，不是 `"PNG+textChunk"`）
- 对话图片规格：**1800×2400 像素，调色板模式（P），2×15 分片，每帧 900×160**
- 肉眼确认：部分帧包含**白色文字**，文字与角色肖像融合在同一图层

这是 Maya 逆向中第一次实质性提取到文本载体。提取路径已打通：运行时解密 → 导出资 PNG → OCR 提取原文。

注入路径分析：

- `project.json` 中无对话文本 → **不能通过修改 JSON 注入译文**
- 解密后的 PNG 无文本元数据 → **不能通过修改元数据注入**
- 图片重新加密密钥未知（`info.json` 的 key 不是 XXTEA 密钥）→ **不能离线加密替换**
- **可行方案**：运行时 Hook `getDataFromFile()`，当游戏请求对话图片时，返回预先生成的译文 PNG。引擎的 `getDataFromFile` 对无 `enc` 头的文件直接返回原始数据，因此译文图片无需加密。
- 译文图片生成：OCR 提取原文 → 翻译 → Pillow 渲染中文 + 合成到原图 → 打包为同规格 PNG

下一步需确认：引擎是否接受非加密的替代图片（即 Hook 或直接放置明文 PNG 是否生效）。

如果导出的文件是标准 PNG，下一步就进入图片切片提取、OCR 和中文 PNG 回写实验。第一轮可以只替换 `img/1394.png` 做最小实机验证，确认 plaintext PNG 是否能被引擎接受。
